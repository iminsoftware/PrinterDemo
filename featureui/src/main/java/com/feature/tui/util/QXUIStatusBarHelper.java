 ...
 * @date 2016-03-27
 */
public class QXUIStatusBarHelper {
    ...
    private static boolean supportTranslucent() {
        return Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT
                ...
                && !(QXUIDeviceHelper.isEssentialPhone() && Build.VERSION.SDK_INT < 26);
    }
    ...
    public static void translucent(Window window, @ColorInt int colorOn5x) {
        ...
        if (QXUINotchHelper.isNotchOfficialSupport()) {
            handleDisplayCutoutMode(window);
        }

        // 小米和魅族4.4 以上版本支持沉浸式
        // 小米 Android 6.0 ，开发版 7.7.13 及以后版本设置黑色字体又需要 clear FLAG_TRANSLUCENT_STATUS, 因此还原为官方模式
        if (QXUIDeviceHelper.isFlymeLowerThan(8) || (QXUIDeviceHelper.isMIUI() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M)) {
...
//            }
    ...
    @TargetApi(28)
    public static void realHandleDisplayCutoutMode(Window window, View decorView) {
    ...
    public static boolean setStatusBarLightMode(Activity activity) {
        ...
        if (QXUIDeviceHelper.isZTKC2016()) {
            return false;
        }
     ...
     */
    public static boolean setStatusBarLightMode(Activity activity, @StatusBarType int type) {
    ...
    public static boolean Android6SetStatusBarLightMode(Window window, boolean light) {
        ...
        if (QXUIDeviceHelper.isMIUIV9()) {
            // MIUI 9 低于 6.0 版本依旧只能回退到以前的方案
            // https://github.com/Tencent/QXUI_Android/issues/160
    ...
    public static boolean isMIUICustomStatusBarLightModeImpl() {
        if (QXUIDeviceHelper.isMIUIV9() && Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
            return true;
        }
        return QXUIDeviceHelper.isMIUIV5() || QXUIDeviceHelper.isMIUIV6() ||
                QXUIDeviceHelper.isMIUIV7() || QXUIDeviceHelper.isMIUIV8();
    }
    ...
    public static boolean FlymeSetStatusBarLightMode(Window window, boolean light) {
        ...
        if (window != null) {
            ...
            if (QXUIDeviceHelper.isFlymeLowerThan(7)) {
                try {
            ...
            } else if (QXUIDeviceHelper.isFlyme()) {
                result = true;
            }
        }
        return result;
    ...
    public static boolean supportTransclentStatusBar6() {
        return !(QXUIDeviceHelper.isZUKZ1() || QXUIDeviceHelper.isZTKC2016());
    }
    ...
    public static void initStatusBarHeight(Context context) {
        ...
        if (sStatusBarHeight <= 0) {
            if (sVirtualDensity == -1) {
                sStatusBarHeight = XUiDisplayHelper.dp2px(context, STATUS_BAR_DEFAULT_HEIGHT_DP);
            } else {
