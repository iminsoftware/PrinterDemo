...
public class IndexableLayout extends FrameLayout {
    ...
    private OnTouchListener mOnIndexBarTouchListener;
    /**
    ...
    private int mMDOverlayGravity;

    private int mBarTextColor, mBarFocusTextColor;
    private float mBarTextSize, mBarTextSpace, mBarWidth, mBarFocusTextBgWidth, mBarFocusTextBgHeight;
    private Drawable mBarBg, mBarFocusTextBg;
...
//    }
    ...
    /**
     * @param gravity Gravity.TOP or  Gravity.TOP or  Gravity.TOP
     */
    public void setOverlayStyle_MaterialDesign(TextView overlayView, int width, int height, int rightMargin, int gravity) {
        mMDOverlayGravity = gravity;

        mMDOverlay = overlayView;
    ...
    public void setOnIndexBarTouchListener(OnTouchListener listener) {
        this.mOnIndexBarTouchListener = listener;
    }
...
//        PADDING_RIGHT_OVERLAY = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 80, getResources().getDisplayMetrics());
        ...
        if (attrs != null) {
            @SuppressLint("CustomViewStyleable") TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.IndexAbleLayout);
            mBarTextColor = a.getColor(R.styleable.IndexAbleLayout_indexBar_textColor, 0xFF999999);
            mBarTextSize = a.getDimension(R.styleable.IndexAbleLayout_indexBar_textSize, getResources().getDimension(R.dimen.dp_10));
            mBarFocusTextColor = a.getColor(R.styleable.IndexAbleLayout_indexBar_selectedTextColor, 0xFF08C5AC);
            mBarTextSpace = a.getDimension(R.styleable.IndexAbleLayout_indexBar_textSpace, getResources().getDimension(R.dimen.dp_4));
            mBarBg = a.getDrawable(R.styleable.IndexAbleLayout_indexBar_background);
            mBarFocusTextBg = a.getDrawable(R.styleable.IndexAbleLayout_indexBar_selectedTextBackground);
            mBarWidth = a.getDimension(R.styleable.IndexAbleLayout_indexBar_layout_width, getResources().getDimension(R.dimen.dp_24));
            mBarFocusTextBgWidth = a.getDimension(R.styleable.IndexAbleLayout_indexBar_selectedTextBackgroundWidth, getResources().getDimension(R.dimen.dp_18));
            mBarFocusTextBgHeight = a.getDimension(R.styleable.IndexAbleLayout_indexBar_selectedTextBackgroundHeight, getResources().getDimension(R.dimen.dp_18));
            a.recycle();
        }
        ...
        mIndexBar.setFocusTextBackground(mBarFocusTextBg, mBarFocusTextBgWidth, mBarFocusTextBgHeight);
        ...
        tv.setBackgroundResource(R.drawable.ic_common_lv_letters_selected);
        tv.setTextColor(0xFFFFFFFF);
        tv.setTextSize(TypedValue.COMPLEX_UNIT_PX, resources.getDimension(R.dimen.dp_24));
        tv.setSingleLine();
        tv.setIncludeFontPadding(false);
        tv.setGravity(Gravity.CENTER_VERTICAL);
        tv.setPadding(resources.getDimensionPixelSize(R.dimen.dp_10), 0, 0, 0);
        setOverlayStyle_MaterialDesign(
                tv,
                resources.getDimensionPixelSize(R.dimen.dp_40),
                resources.getDimensionPixelSize(R.dimen.dp_35),
                resources.getDimensionPixelSize(R.dimen.dp_28),
                Gravity.CENTER
        );
    ...
    private void initListener() {
        ...
        mIndexBar.setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View v, MotionEvent event) {
                if (mOnIndexBarTouchListener != null) mOnIndexBarTouchListener.onTouch(v, event);
    ...
    private void showOverlayView(float yInBar, final int touchPos) {
        if (mIndexBar.getIndexList().size() <= touchPos) return;

        if (mMDOverlay != null) {
            if (mMDOverlay.getVisibility() != VISIBLE) mMDOverlay.setVisibility(VISIBLE);

            int overlayHeight = mMDOverlay.getLayoutParams().height;
            float y = mIndexBar.getTop() + yInBar;

            int halfIndexHeight = (mIndexBar.getHeight() / mIndexBar.getIndexList().size()) / 2;
            int yBarTopBorder = mIndexBar.getTop() + halfIndexHeight;
            int yBarBottomBorder = mIndexBar.getBottom() - halfIndexHeight;
            y = y < yBarTopBorder ? yBarTopBorder : y > yBarBottomBorder ? yBarBottomBorder : y;

            if (mMDOverlayGravity == Gravity.CENTER) y -= overlayHeight / 2;
            else if (mMDOverlayGravity == Gravity.BOTTOM) y -= overlayHeight;

            int yTopBorder = 0;
            int yBottomBorder = getHeight() - overlayHeight;
            y = y < yTopBorder ? yTopBorder : y > yBottomBorder ? yBottomBorder : y;

            mMDOverlay.setY(y);

            String index = mIndexBar.getIndexList().get(touchPos);
            if (!mMDOverlay.getText().equals(index)) mMDOverlay.setText(index);
        }
        if (mCenterOverlay != null) {
            if (mCenterOverlay.getVisibility() != VISIBLE) mCenterOverlay.setVisibility(VISIBLE);
            String index = mIndexBar.getIndexList().get(touchPos);
            if (!mCenterOverlay.getText().equals(index)) {
                if (index.length() > 1) mCenterOverlay.setTextSize(32);
    ...
    private <T extends IndexableEntity> ArrayList<EntityWrapper<T>> transform(final List<T> datas) {
        try {
            ...
            for (List<EntityWrapper<T>> indexableEntities : map.values()) {
                ...
                for (int i = 0; i < indexableEntities.size(); i++) {
                    T data = indexableEntities.get(i).getData();
                    if (data != null)
                        data.onSortFinished(i == 1, i == indexableEntities.size() - 1);
                }
