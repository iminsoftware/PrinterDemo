package com.feature.tui.widget.ratingbar;
 ...
 */
public class XUIRatingBar extends View {
  ...
  public XUIRatingBar(Context context) {
    super(context);
    initView();
  }

  public XUIRatingBar(Context context, AttributeSet attrs) {
    super(context, attrs);
    parseAttrs(attrs);
    initView();
  }

  public XUIRatingBar(Context context, AttributeSet attrs, int defStyleAttr) {
  ...
  private void parseAttrs(AttributeSet attrs) {
    TypedArray arr = getContext().obtainStyledAttributes(attrs, R.styleable.XUIRatingBar);

    borderColor = arr.getColor(R.styleable.XUIRatingBar_rb_borderColor, getResources().getColor(R.color.xui_config_color_main));
    fillColor = arr.getColor(R.styleable.XUIRatingBar_rb_fillColor, borderColor);
    starBackgroundColor = arr.getColor(R.styleable.XUIRatingBar_rb_starBackgroundColor, Color.TRANSPARENT);
    backgroundColor = arr.getColor(R.styleable.XUIRatingBar_rb_backgroundColor, Color.TRANSPARENT);

    pressedBorderColor = arr.getColor(R.styleable.XUIRatingBar_rb_pressedBorderColor, borderColor);
    pressedFillColor = arr.getColor(R.styleable.XUIRatingBar_rb_pressedFillColor, fillColor);
    pressedStarBackgroundColor = arr.getColor(R.styleable.XUIRatingBar_rb_pressedStarBackgroundColor, starBackgroundColor);
    pressedBackgroundColor = arr.getColor(R.styleable.XUIRatingBar_rb_pressedBackgroundColor, backgroundColor);

    numberOfStars = arr.getInteger(R.styleable.XUIRatingBar_rb_numberOfStars, 5);

    starsSeparation = arr.getDimensionPixelSize(R.styleable.XUIRatingBar_rb_starsSeparation, (int)valueToPixels(4, Dimension.DP));
    maxStarSize = arr.getDimensionPixelSize(R.styleable.XUIRatingBar_rb_maxStarSize, Integer.MAX_VALUE);
    desiredStarSize = arr.getDimensionPixelSize(R.styleable.XUIRatingBar_rb_starSize, Integer.MAX_VALUE);
    stepSize = arr.getFloat(R.styleable.XUIRatingBar_rb_stepSize, 0.1f);
    starBorderWidth = arr.getFloat(R.styleable.XUIRatingBar_rb_starBorderWidth, 5f);
    starCornerRadius = arr.getFloat(R.styleable.XUIRatingBar_rb_starCornerRadius, 6f);

    rating = normalizeRating(arr.getFloat(R.styleable.XUIRatingBar_rb_rating, 0f));
    isIndicator = arr.getBoolean(R.styleable.XUIRatingBar_rb_isIndicator, false);
    drawBorderEnabled = arr.getBoolean(R.styleable.XUIRatingBar_rb_drawBorderEnabled, true);
    gravity = Gravity.fromId(arr.getInt(R.styleable.XUIRatingBar_rb_gravity, Gravity.Left.id));

    mIsContinue = arr.getBoolean(R.styleable.XUIRatingBar_rb_continuous, true);
  ...
  private void animateRating(AnimationBuilder builder) {
    ...
    ratingAnimator.addListener(new AnimatorListener() {
      ...
      public void onAnimationEnd(Animator animator) {
        if (ratingListener != null) {
          ratingListener.onRatingChanged(XUIRatingBar.this, rating, false);
        }
      }
  ...
  private float normalizeRating(float rating) {
    if (rating < 0) {
      ...
      if (mIsContinue) return rating;
      else {
        return (float) Math.ceil(rating);
      }
    }
  }
  ...
  public interface OnRatingBarChangeListener {
     ...
     * lifting the touch.
     *
     * @param XUIRatingBar The RatingBar whose rating has changed.
     * @param rating The current rating. This will be in the range
     *            0..numStars.
     * @param fromUser True if the rating change was initiated by a user's
     *            touch gesture or arrow key/horizontal trackbell movement.
     */
    void onRatingChanged(XUIRatingBar XUIRatingBar, float rating, boolean fromUser);
  ...
  public class AnimationBuilder {
    private XUIRatingBar XUIRatingBar;
    ...
    private AnimationBuilder(XUIRatingBar XUIRatingBar) {
      this.XUIRatingBar = XUIRatingBar;
      this.duration = 2000;
      this.interpolator = new BounceInterpolator();
      this.ratingTarget = XUIRatingBar.getNumberOfStars();
      this.repeatCount = 1;
     ...
     */
    public void start() {
      XUIRatingBar.animateRating(this);
    }
  }
}
