...
public class IndexBar extends View {
    ...
    private float mFocusTextBgWidth, mFocusTextBgHeight;
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mIndexList.size(); i++) {
            ...
            int top = (int) (mIndexHeight * i);
            int baseline = (int) (top + (mIndexHeight - fontMetrics.bottom + fontMetrics.top) / 2 - fontMetrics.top);
            int centerX = getWidth() / 2;

            if (mSelectionPosition == i && mFocusTextBg != null) {
                int centerY = (int) (top + mIndexHeight / 2);
                int offsetX = (int) (mFocusTextBgWidth / 2);
                int offsetY = (int) (mFocusTextBgHeight / 2);
                mFocusTextBg.setBounds(centerX - offsetX, Math.max(centerY - offsetY, 0), centerX + offsetX, Math.min(centerY + offsetY, getHeight()));
                mFocusTextBg.draw(canvas);
            }
            canvas.drawText(mIndexList.get(i), centerX, baseline, mSelectionPosition == i ? mFocusPaint : mPaint);
//            canvas.drawText(text, getWidth() / 2, mIndexHeight * 0.85f + mIndexHeight * i, mPaint);
        }
    }
    ...
    int getFirstRecyclerViewPositionBySelection() {
        ...
        if (mMapping.containsKey(index)) {
            Integer position = mMapping.get(index);
    ...
    void setDatas(boolean showAllLetter, ArrayList<EntityWrapper> datas) {
        ...
        if (showAllLetter) {
            mIndexList = Arrays.asList(getResources().getStringArray(R.array.index_letter));
    ...
    void setFocusTextBackground(Drawable background, float width, float height) {
        mFocusTextBg = background;
        mFocusTextBgWidth = width;
        mFocusTextBgHeight = height;
    }
