    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (viewType == EntityWrapper.TYPE_TITLE) {
            if (mTitleClickListener != null) {
                onClickListener = (v -> mTitleClickListener.onItemClick(v, position, wrapper.getIndexTitle()));
            }
            if (mTitleLongClickListener != null) {
                onLongClickListener = (v -> mTitleLongClickListener.onItemLongClick(v, position, wrapper.getIndexTitle()));
            }
        } else if (viewType == EntityWrapper.TYPE_CONTENT) {
            if (mContentClickListener != null) {
                onClickListener = (v -> mContentClickListener.onItemClick(v, wrapper.getOriginalPosition(), position, wrapper.getData()));
            }
            if (mContentLongClickListener != null) {
                onLongClickListener = (v -> mContentLongClickListener.onItemLongClick(v, wrapper.getOriginalPosition(), position, wrapper.getData()));
            }
        } else {
            AbstractHeaderFooterAdapter adapter;
            if (mHeaderAdapterMap.indexOfKey(viewType) >= 0) {
            ...
            if (adapter != null) {
                ...
                if (listener != null) {
                    onClickListener = (v -> listener.onItemClick(v, position, wrapper.getData()));
                }
                AbstractHeaderFooterAdapter.OnItemLongClickListener longListener = adapter.getOnItemLongClickListener();
                if (longListener != null) {
                    onLongClickListener = (v -> longListener.onItemLongClick(v, position, wrapper.getData()));
                }
            }
        }
        holder.itemView.setOnClickListener(onClickListener);
        holder.itemView.setClickable(onClickListener != null);
        holder.itemView.setOnLongClickListener(onLongClickListener);
        holder.itemView.setLongClickable(onLongClickListener != null);
    }
