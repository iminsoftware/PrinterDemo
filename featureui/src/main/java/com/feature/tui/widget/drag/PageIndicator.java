...
public class PageIndicator extends ViewGroup {
    ...
    public PageIndicator(Context context, AttributeSet attrs) {
        ...
        mPageIndicatorWidth =
                (int) mContext.getResources().getDimension(R.dimen.drag_page_indicator_width);
        mPageIndicatorHeight =
                (int) mContext.getResources().getDimension(R.dimen.drag_page_indicator_height);
...
//                (index + 1), getChildCount()));
    ...
    private void setPosition(int position) {
        if(Math.abs(mPosition - position) >= 2) {
